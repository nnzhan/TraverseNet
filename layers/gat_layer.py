...
class GATConvs(nn.Module):
    ...
    def __init__(self,
                 ...
                 residual=False,
                 activation=None,
                 allow_zero_in_degree=False,fix=False):
    ...
    def forward(self, graph, feat):
        ...
        with graph.local_scope():
            ...
            if isinstance(feat, tuple):
                ...
                shape = h_src.shape
                feat_src = self.fc1(h_src).view(
                    *shape[:-1], self._num_heads, self._out_feats)
                feat_src1 = self.fc1(h_src).view(
                    *shape[:-1], self._num_heads, self._out_feats)
                feat_dst = self.fc1(h_src).view(
                    *shape[:-1], self._num_heads, self._out_feats)
                if graph.is_block:
            ...
            graph.srcdata.update({'ft': feat_src1, 'el': el})
            ...
            if self.fix:
                e = th.ones_like(e).to(e.device)
            # compute softmax
            #graph.edata['a'] = self.attn_drop(edge_weights)
            ...
            #     sn, tg = graph.edges()
            #     data = dict()
            #     data['a'] = graph.edata['a'].cpu()
            #     data['src'] = sn.cpu()
            #     data['tgt'] = tg.cpu()
            #     file = open('./att_l' + str(self.layerid) + "_" + str(int(np.random.random() * 10000)) + '.pkl', "wb")
